...
{% set active_ids_exist = adapter.get_relation(
        ...
        schema=this.schema,
        identifier="github_reactions_dlt_processed_load_ids" ) %}
...
AND load_id NOT IN (SELECT load_id FROM {{ source('transformed_data', 'github_reactions_dlt_processed_load_ids') }})
{% endif %}